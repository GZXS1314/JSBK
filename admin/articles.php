...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'save_article') {
    ...
    if ($title && $content) {
        if ($id > 0) {
            $stmt = $pdo->prepare("UPDATE articles SET title=?, content=?, summary=?, category=?, cover_image=?, media_type=?, media_data=?, is_recommended=?, is_hidden=? WHERE id=?");
            $stmt->execute([$title, $content, $summary, $category, $cover_image, $media_type, $media_data, $is_recommended, $is_hidden, $id]);
            $pdo->prepare("DELETE FROM tags WHERE article_id=?")->execute([$id]);
            $article_id = $id;
        } else {
            $stmt = $pdo->prepare("INSERT INTO articles (title, content, summary, category, cover_image, media_type, media_data, is_recommended, is_hidden) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$title, $content, $summary, $category, $cover_image, $media_type, $media_data, $is_recommended, $is_hidden]);
