...
if ($slides === false) {
    ...
    // 将结果写入缓存，有效期10分钟 (600秒)，因为推荐文章可能变化更频繁
    Cache::set('articles_recommended', $slides, 600);
}


// --- 2. 获取首页特有的配置数据 (这些数据已在 header.php 中通过 site_config 获取，无需重复查询) ---
            ...
            <?php else: ?>
                ...
                <div class="slider-track" id="sliderTrack">
                    ...
                    <a href="javascript:void(0)" onclick="openArticle(<?= $slide['id'] ?>)" class="slider-item">
                        <div class="slide-img-wrap"><img src="<?= htmlspecialchars(!empty($slide['cover_image']) ? $slide['cover_image'] : 'https://placehold.co/800x600?text=No+Image') ?>" alt="<?= htmlspecialchars($slide['title']) ?>"></div>
                        <div class="slide-content"><span class="slide-tag"><?= htmlspecialchars($slide['category']) ?></span><h3 class="slide-title"><?= htmlspecialchars($slide['title']) ?></h3><span class="slide-meta"><?= $slide['views'] ?> views · <?= date('Y-m-d', strtotime($slide['created_at'])) ?></span></div>
                    </a>
                    <?php endforeach; ?>
                </div>
...
<div class="modal-overlay" id="articleModal">
    <div class="modal-card">
        <div class="modal-header-bar">
             <div class="modal-header-title" id="modalHeaderTitle">文章详情</div>
            <div class="close-modal-btn" onclick="closeModal()"><i class="fa-solid fa-xmark"></i></div>
        </div>
